...
class TransformerSentenceEncoder(nn.Module):
    ...
    def __init__(
        ...
        if int(add_know[0]) >= 0:
            self.kembed_tokens = deepcopy(self.embed_tokens)
        else:
            self.kembed_tokens = None
        if q_noise > 0:
        ...
        self.layers.extend(
            [
                self.build_transformer_sentence_encoder_layer(
                    ...
                    add_know=True if (i >= int(add_know[0]) and i < int(add_know[1])) else False,
                )
    ...
    def forward(
        ...
        question_concepts: Optional[torch.Tensor] = None,
        relation_concepts: Optional[torch.Tensor] = None,
        ...
        if knowledge is not None:
            ...
            origin_knowledge_mask = knowledge.eq(self.padding_idx)
            knowledge = self.kembed_tokens(knowledge)
            ...
            avg_embedding = x.mean(axis=1).unsqueeze(-1).detach()
        ...
        for layer in self.layers:
            x, _ = layer(x, self_attn_padding_mask=padding_mask, knowledge=knowledge)
            if not last_state_only:
                inner_states.append(x)
        sentence_rep = x[0, :, :]
