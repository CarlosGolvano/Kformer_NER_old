...
@register_task("social_iqa")
class SocialIQATask(LegacyFairseqTask):
    ...
    def load_dataset(
        ...
        with open(data_path) as h:
            for line in h:
                ...
                question = "Q: " + question
                context_toks = binarize(context, append_bos=True)
                question_toks = binarize(question)
                for i, choice in enumerate(example["choices"]):
                    src = "A: " + choice["text"]
                    src_bin = torch.cat([context_toks, question_toks, binarize(src)])
