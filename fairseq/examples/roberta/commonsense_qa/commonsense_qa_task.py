...
class CommonsenseQATask(LegacyFairseqTask):
    ...
    def load_dataset(
        ...
        relation_concepts = [[] for i in range(self.args.num_classes)]
        labels = []
        max_know_length = 18
        with open(data_path) as h:
            for line in h:
                ...
                question_bin = binarize(example["question"]["question_concept"], append_eos=False)
                ...
                for i, choice in enumerate(example["question"]["choices"]):
                    ...
                    # knowledge_text =  ""
                    triple = ""
                    if len(choice["triple"]) > 0:
                        triples = choice["triple"][0]
                        triple = " ".join(triples)
                        # for rela in triples:
                        #     triple += rela + " "
                    if len(triple)<=0:
                        triple = example["question"]["question_concept"] + " " + choice["text"]
                    relation_bin = binarize(triple, append_eos=False)
                    knowledges = choice["knowledge"]
                    while len(knowledges)<20:
                        knowledges.append(example["question"]["question_concept"]+" "+choice["text"])
                    know_bin = []
                    ...
                    # knowledge_bin = binarize(triple, append_eos=False)
                    src_bin = torch.cat([question_toks, binarize(src)])
                    knowledge[i].append(knowledge_bin)
                    src_tokens[i].append(src_bin)
                    relation_concepts[i].append(relation_bin)
        ...
        for i in range(self.args.num_classes):
            ...
            relation_concepts[i] = ListDataset(relation_concepts[i])
            # knowledge[i] = ListDataset(knowledge[i])
        dataset = {
            "id": IdDataset(),
            ...
            dataset.update(
                {
                    "net_input{}".format(i + 1): {
                        ...
                        # "knowledge":RightPadDataset(
                        #     knowledge[i],
                        #     pad_idx=self.source_dictionary.pad(),
                        # ),
                        "relation_concepts":RightPadDataset(
                            relation_concepts[i],
                            pad_idx=self.source_dictionary.pad(),
                        ),
